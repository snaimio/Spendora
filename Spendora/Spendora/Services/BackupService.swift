...
class BackupService {
    ...
    func exportBackup(subscriptions: [Subscription]) -> URL? {
        ...
        do {
            ...
            let url = FileManager.default.temporaryDirectory.appendingPathComponent("Spendora_Backup_\(Date().timeIntervalSince1970).json")
    ...
    func importBackup(from url: URL, modelContext: ModelContext) throws -> Int {
        ...
        for item in json {
            guard let name = item["name"] as? String,
                  let cost = item["cost"] as? Double else { continue }
            ...
            if let timestamp = item["trialEndDate"] as? TimeInterval, timestamp > 0 {
