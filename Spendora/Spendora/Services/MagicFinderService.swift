...
class MagicFinderService {
    ...
    func detectFromEmailBody(_ body: String) -> [String] {
        let detected = detectSubscriptions(from: body)
        return detected
    }
    ...
    func extractAmount(from text: String) -> Double? {
        ...
        for result in results ?? [] {
            let amount = nsString.substring(with: result.range(at: 1))
    ...
    func quickAddFromText(_ text: String) -> (name: String?, cost: Double?, category: String?) {
    ...
    private func detectCategory(from text: String) -> String? {
        let lowercased = text.lowercased()
        if lowercased.contains("music") || lowercased.contains("tv") || lowercased.contains("movie") || lowercased.contains("show") {
            return "Entertainment"
        } else if lowercased.contains("work") || lowercased.contains("office") || lowercased.contains("cloud") || lowercased.contains("storage") {
            return "Productivity"
        } else if lowercased.contains("fitness") || lowercased.contains("health") || lowercased.contains("gym") || lowercased.contains("workout") {
            return "Health & Fitness"
        } else if lowercased.contains("shop") || lowercased.contains("delivery") || lowercased.contains("prime") {
            return "Shopping"
        } else if lowercased.contains("food") || lowercased.contains("meal") || lowercased.contains("cook") {
            return "Food & Dining"
        } else if lowercased.contains("learn") || lowercased.contains("course") || lowercased.contains("class") {
            return "Education"
        }
        return nil
    }
