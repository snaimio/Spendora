...
struct SavingsScoreView: View {
    ...
    private func generateShareImage() {
        let renderer = ImageRenderer(content: ShareableScoreCard(score: savingsScore, count: subscriptions.count, total: subscriptions.reduce(0) { $0 + $1.monthlyCost }))
...
struct ShareableScoreCard: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                Label("\(count) subscriptions", systemImage: "number.circle.fill")
                    .font(.body)
                    .foregroundColor(.secondary)
                
                Label(CurrencyManager.shared.format(total) + "/month", systemImage: "dollarsign.circle.fill")
                    .font(.body)
                    .foregroundColor(.secondary)
            }
